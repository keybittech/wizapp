...
export async function guidedEdit(fileParts: string, editingUser?: string) {
  ...
  if (sourceFile) {
    ...
    const functionStatementRegex = /FunctionDeclaration|ArrowFunction|MethodDeclaration/;
    sourceFile.forEachDescendant((node, index) => {
      if (functionStatementRegex.test(node.getKindName())) {
        parsedStatements[`function_statement_${index}`] = node.getFullText();
      } else if (!ignoredStatements.includes(node.getKind())) {
        walkNode(node, index, parsedStatements, originalStatements);
      }
    });
    ...
    Object.keys(generatedStatements).forEach(statementKey => {
      ...
      if (['new_statement', 'newstatement', `statement_${originalStatements.size + 1}`].includes(stKey)) {
        ...
        fileModified = true;
      } else if ([/above_\d{1,2}/, /below_\d{1,2}/, /above_function_statement_\d{1,2}/, /below_function_statement_\d{1,2}/].some(regex => regex.test(stKey))) {
        const [direction, type, index] = stKey.split('_');
        const statements = (type === 'function' ? parsedStatements : originalStatements);
        const adjacentStatement = statements[`statement_${index}`] || statements[`function_statement_${index}`];
        ...
        const originalStatement = (originalStatements.get(stKey) || parsedStatements[stKey]);
